...
- **화면 단위 밖에서 남은 작업(우선순위 순서 없음, 사용자 지정 대기)**:
  1. 기상청 날씨 API 연동 미착수 — `Round.weatherSnapshot`이 항상 null이라 9번/8번 모두 "날씨 정보 없음"만 표시.
  2. `app/README.md`·루트 `README.md`가 구버전 기준으로 멈춰 있음 — 스키마/화면 갱신 내용과 동기화 필요.
  3. 좌표 결측 골프장 지오코딩 배치 — 원본 좌표값 자체가 없는 소수 골프장은 여전히 "위치 확인 중" 상태.
  4. 카카오 OAuth 실제 로그인 테스트 — 사용자가 Kakao Developers에서 앱 등록/키 발급 후 로컬 테스트 예정(구글은 완료).
  5. 하이원CC 홀별 Par 자료 미확보(부차 데이터 이슈) — 사용자가 공식 스코어카드 확보 시 재요청 예정.
...
- 세부 이력은 아래 8~76번 항목(시간순 작업 로그)에 모두 기록되어 있음.
