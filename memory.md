...
## 135. 네이버 로그인 후속 버그 2건 수정 (2026-08-07)

134번에서 네이버 로그인을 추가한 뒤 재홍님이 로컬에서 실제 테스트하며 순차적으로 발견한
문제 2건을 같은 세션에서 수정.

### 1) `Server error - There is a problem with the server configuration`

- 원인: `.env`에 `NAVER_CLIENT_ID`/`SECRET`을 입력한 뒤 `npm run dev`를 재시작하지 않아
  서버가 여전히 빈 값으로 provider를 구성한 상태였음(코드/`.env` 자체는 문제없었음). 재시작
  후 해결 확인.
- 별도 코드 수정 없음 — 다음 세션에서 비슷한 "값은 넣었는데 안 먹는다" 류 이슈가 재발하면
  이 케이스부터 의심할 것.

### 2) `login?error=OAuthAccountNotLinked`

- 원인: 46번 항목에서 이미 예견했던 사안 그대로 — 네이버 계정 이메일이 이메일/비밀번호(또는
  구글)로 이미 가입된 계정과 같아서, Auth.js가 계정 탈취 방지 차원에서 자동 연결을 거부.
- AskUserQuestion으로 처리 방향 확인(자동 연결 허용 / 다른 이메일로 테스트 / 직접 입력) →
  재홍님 "no preference"로 PM 추천안(자동 연결 허용) 채택.
- **수정**: `app/src/auth.config.ts`의 Google·Naver provider 둘 다에
  `allowDangerousEmailAccountLinking: true` 추가. Auth.js가 "dangerous"라 이름 붙인 이유는
  provider가 이메일 소유를 검증했다는 보장이 없을 때의 계정 탈취 위험 때문인데, 구글·네이버
  둘 다 자체 이메일 인증을 거친 계정만 이메일을 내려주고, 이 프로젝트는 개인용 소규모
  서비스라 위험도가 낮다고 판단해 켜기로 결정(정책 자체를 이 항목에서 처음 확정).

### 3) 네이버 로그인 시 `User.name`이 DB에 저장 안 됨

- 원인: next-auth 내장 Naver provider의 기본 `profile()`이 `name`을
  `response.nickname`(별명)에서만 가져오는데, 네이버 개발자센터 앱의 "제공 정보 선택"에서
  별명 동의 항목이 없거나 사용자가 비허용한 경우 `nickname` 필드 자체가 응답에 안 실려
  옴 → `name`이 `undefined`로 매핑되어 `PrismaAdapter.createUser()`가 `null`로 저장.
- **수정**: `auth.config.ts`의 Naver provider에 `profile()`을 직접 재정의해
  `response.name(실명) ?? response.nickname(별명) ?? null` 순서로 폴백하도록 변경.
- **한계**: 이 수정은 **이후 신규 가입자**부터 적용됨. 재홍님이 이미 테스트하면서 생성된
  `name=null` User 행은 소급 반영이 안 됨(Auth.js는 계정 연결/로그인 시 기존 User 레코드의
  name을 재갱신하지 않는 설계) — Supabase에서 해당 행을 직접 `UPDATE`하거나, 테스트용
  계정이면 그냥 삭제 후 재가입(재로그인)으로 새로 만드는 게 더 간단.
- 검증: `npx tsc --noEmit` 클린. 두 수정 모두 실제 로그인 성공 여부·이름 저장 여부는
  재홍님 로컬 재확인 필요.

### 4) `User.image`도 null로 저장됨 (코드 문제 아님)

재홍님이 이름은 저장됐는데 `image`가 null이라고 후속 보고. 코드 확인 결과
`profile.response.profile_image → image` 매핑은 원래부터 정상(안 건드림) — 3)과 동일한
패턴으로, **네이버 개발자센터 앱의 "제공 정보 선택"에서 프로필 사진 항목이 비활성이거나
계정에 사진 자체가 없어서** 응답에 필드가 안 실려 오는 것으로 진단. 코드 수정 없음.

로그인마다 name/image가 비어있으면 최신값으로 자동 채워주는 이벤트 훅(백필) 추가를
제안했으나 **재홍님이 "지금 로직 유지"로 결정** — 추가하지 않음. 즉 기존 정책(신규 가입
시점에만 profile() 값을 저장, 이후 로그인에서는 기존 User 레코드를 갱신하지 않음) 그대로
...
- 재홍님이 (a) 네이버 로그인이 정상적으로 되는지, (b) 새로 가입/연결한 계정의 이름이
  제대로 저장되는지 확인 결과를 알려주면 이 작업 단위(134~135번) 완전히 종료.
  `image` 필드 백필 로직은 이번 세션에 명시적으로 "추가 안 함"으로 결정됐으므로, 나중에
  다시 요청받기 전까지는 제안하지 않을 것. 기존에 생성된 `name`/`image` null 테스트 행
  정리는 재홍님이 Supabase에서 직접 처리하기로 함.


## 136. 네이버 서비스 로고 이미지 제작 (2026-08-07)

네이버 로그인 정상 동작 확인 후, 재홍님이 네이버 개발자센터 등록용 서비스 로고 이미지
요청(권장 140x140, 500KB 이하, jpg/png/gif).

- 프로젝트 브랜드 컬러(`globals.css`의 `--color-primary`(#1f4d2c 짙은 초록)/`--color-accent`
  (#d99a3c 골드))를 그대로 활용해 골프 깃대+공 아이콘을 SVG로 직접 제작 → cairosvg로
  140x140 PNG 래스터화(투명 배경 없이 전체 정사각형을 배경색으로 채워 알파 채널 문제 예방).
  최종 파일 크기 약 7KB(500KB 제한 대비 여유 충분).
- 산출물: `doc/assets/naver-service-logo-140.png`(등록용), `doc/assets/naver-service-logo.svg`
  (원본, 추후 파비콘/다른 브랜딩 소재로 재사용 가능).
- 이 프로젝트는 아직 파비콘/앱 아이콘 자체가 없는 상태였음(`app/public`에 로고 파일 없음) —
  이번에 만든 로고를 파비콘으로도 재사용할지는 재홍님 판단에 맡기고, 이번 범위(네이버 등록용)
  로는 확장하지 않음.

### 다음 세션 시작 시

- 재홍님이 네이버 개발자센터에 이 로고를 업로드한 결과(승인/반려 여부) 알려주면 종료.
  반려되면 네이버 쪽 반려 사유 기준으로 재작업.


## 137. 로고 샘플 2종 추가 (2026-08-07)

136번(골프 깃대+공)에 이어 재홍님이 "score caddie 컨셉에 맞게" 샘플 2개 추가 요청 —
같은 브랜드 컬러(#1f4d2c/#6fa167/#d99a3c/#f4efe6)와 140x140 정사각 배경 규격을 유지하되
모티프만 다르게 제작.

- `doc/logo/scorecard-logo-140.png`(+ .svg): "Score" 컨셉 — 스코어카드(클립보드) +
  체크마크 배지.
- `doc/logo/golfbag-logo-140.png`(+ .svg): "Caddie" 컨셉 — 골프백 + 클럽 2개.
- 기존 136번 결과물은 `doc/logo/naver-service-logo-140.png`로 이미 옮겨져 있어(재홍님
  요청으로 `doc/assets`→`doc/logo` 이동), 총 3종이 `doc/logo/`에 나란히 있음. 네이버
  등록은 이 중 하나만 쓰면 되고 나머지는 향후 파비콘/다른 브랜딩 용도 후보로 남겨둠.
- 셋 다 cairosvg로 140x140 RGB(투명 없음) PNG 렌더링, 파일 크기 6~7KB 수준(500KB 제한
  대비 여유 충분).

### 다음 세션 시작 시

- 재홍님이 3종 중 실제 네이버 등록에 쓴 것/마음에 드는 것을 알려주면 기록. 특별한 후속
  조치 요청 없으면 이 항목은 종료.


## 138. 스코어카드 로고에 "골프" 표시 추가 (2026-08-07)

재홍님이 137번 스코어카드 로고를 보고 "이건 골프 스코어 카드라는 표시가 있었으면"
피드백 — 기존엔 클립보드+체크마크만 있어 골프 연관성이 약했음.

- **수정**: 카드 상단에 골프 깃발(핀+삼각 깃발)을 클립처럼 꽂아 넣고, 그 아래 카드 안에
  "GOLF" 텍스트 라벨(진초록, 볼드)을 추가. 스코어 라인·체크마크 배지는 그대로 유지.
  텍스트는 DejaVu Sans 폴백으로 렌더링됨(샌드박스에 Arial 없음, `font-family` 우선순위엔
  Arial/Helvetica 그대로 남겨둠 — 실제 브라우저/OS에서 열면 시스템 폰트로 정상 렌더링).
- `doc/logo/scorecard-logo-140.png`/`.svg` 덮어쓰기 완료. 나머지 2종(깃대+공, 골프백)은
  변경 없음.
...
## 140. Hydration 에러 — Grammarly 확장 프로그램 간섭 (2026-08-07)

재홍님이 콘솔에 "Hydration failed..." 에러 보고. 에러 메시지에 포함된
`data-new-gr-c-s-check-loaded`/`data-gr-ext-installed` 속성이 결정적 단서 — 이건 우리
코드가 아니라 **브라우저에 설치된 Grammarly 확장 프로그램**이 React 하이드레이션 되기
전에 `<body>` 태그에 주입하는 속성이라, 서버가 그린 HTML과 클라이언트가 그린 HTML이
(그 속성 유무만) 달라 보여서 나는 경고. 파비콘(139번) 작업과는 무관 — 원래부터 있었는데
이번에 처음 눈에 띈 것으로 보임.

- **수정**: `app/src/app/layout.tsx`의 `<body>`에 `suppressHydrationWarning` 추가. React가
  이 태그 하나의 속성 불일치만 무시하도록 함(Next.js 공식 문서에서도 브라우저 확장 간섭
  케이스의 표준 대응으로 안내하는 방식) — 자식 트리 전체나 다른 컴포넌트에는 영향 없음.
- 검증: `npx tsc --noEmit` 클린. **재홍님 로컬에서 Grammarly 켠 채로 콘솔에 더는 이 에러가
  안 뜨는지 확인 필요**(Grammarly 꺼진 브라우저/시크릿 창에서는 애초에 재현이 안 될 수
  있음 — 재현 조건 자체가 확장 설치 여부).

**결과(2026-08-07)**: 재홍님이 로컬 재확인 — Hydration 에러 사라짐 확인 완료. 140번
항목 종료.

### 다음 세션 시작 시

- 특별한 후속 요청 없으면 로고/파비콘/hydration 작업 단위(136~140번) 전체 종료.


## 141. 회원 탈퇴 기능 추가 (2026-08-07)

재홍님이 "마이페이지에 회원 탈퇴 기능 추가, 회원탈퇴시 회원과 관련된 모든 자료 삭제" 요청.

- **설계**: `schema.prisma`를 다시 확인해보니 `Account`/`Session`/`Round`가 전부 `User`에
  `onDelete: Cascade`로 이미 연결돼 있고, `HoleScore`도 `Round`에 `onDelete: Cascade`로
  연결돼 있어 — **스키마 변경이나 별도 삭제 루프 없이 `prisma.user.delete()` 한 번이면
  회원 관련 데이터(연동 계정, 세션, 라운드, 홀스코어)가 전부 함께 삭제**됨. `GolfCourse`류
  (공용 골프장 데이터)는 User와 무관한 별도 참조라 영향 없음 — 설계 단계에서 가장 중요했던
  확인 포인트.
- **본인 확인 방식**: 이메일/비밀번호 계정은 46/704번 항목과 같은 패턴으로 `passwordHash`
  존재 여부로 분기 — 있으면 비밀번호 재입력, 없으면(소셜 전용) 비밀번호가 없으므로 확인
  문구("탈퇴합니다") 직접 입력으로 대체(코드로 동등한 보안 수준은 아니지만, 이미 이
  프로젝트의 비밀번호 변경 기능도 소셜 계정은 아예 숨기는 방식이라 같은 한계선 안에서
  최선). 문구는 `app/src/lib/constants/account.ts`(`DELETE_ACCOUNT_CONFIRM_PHRASE`)로
  분리해 서버(`api/me/route.ts`)·클라이언트(`DeleteAccountForm.tsx`) 양쪽에서 같은 값을
  가져다 쓰게 함(한쪽만 바뀌는 실수 방지) — `route.ts`에 HTTP 메서드가 아닌 임의 상수를
  직접 export하면 Next.js App Router route handler 규칙과 부딪힐 수 있어 별도 파일로 뺌.
- **구현**:
  - `app/src/app/api/me/route.ts`: 기존 `PATCH`(동의 토글) 옆에 `DELETE` 핸들러 추가.
    비밀번호/확인문구 검증 → `prisma.user.delete()`.
  - `app/src/components/DeleteAccountForm.tsx`(신규): 클라이언트 폼. 성공 시
    `signOut({callbackUrl:"/"})`로 세션 쿠키 즉시 정리 — **주의**: 세션 전략이 `jwt`라
    User가 DB에서 삭제돼도 기존 JWT 쿠키 자체는 만료 전까지 "유효"한 것처럼 남아있을 수
    있어(세션 콜백이 DB 재조회 없이 토큰 내용만 복사), 탈퇴 직후 반드시 클라이언트에서
    로그아웃까지 해줘야 함 — 안 하면 이미 삭제된 사용자로 대시보드 등 화면에서 이상 동작
    가능성.
  - `app/src/app/profile/delete-account/page.tsx`(신규): 서버 컴포넌트로 `passwordHash`
    유무 조회 후 `DeleteAccountForm`에 전달(변경-비밀번호 페이지와 달리, 조건부 UI가
    필요해 서버 컴포넌트로 감쌈).
  - `app/src/app/profile/page.tsx`: 설정 목록에 "회원 탈퇴" 항목 추가(로그아웃과 같은
    경고색 `#D85A30`, 배경은 필박 스타일로 로그아웃보다 강조).
  - `doc/pages.md`(10-2 신규 섹션), `doc/개발리스트.md`(10번 표) 반영.
- **미정 사항으로 남긴 것**: 마지막 남은 관리자(`role=ADMIN`) 계정이 자기 자신을 탈퇴시키는
  경우를 막는 로직은 없음(요청 범위 밖이라 추가 안 함, pages.md 10-2에 명시) — 실제로
  겪으면 그때 정책 논의.
- 검증: `npx tsc --noEmit` 클린. 이 프로젝트엔 `/api/me` 관련 유닛/e2e 테스트가 원래
  없었음(비밀번호 변경 때도 마찬가지) — 이번에도 추가하지 않고 기존 관례를 따름.
...
**결과(2026-08-07)**: 재홍님이 로컬 탈퇴 테스트 완료 확인. 141번 항목 종료.
...
## 143. 구글 로그인 버튼에 브랜드 로고 추가 (2026-08-07)

재홍님이 "네이버·카카오는 브랜드 컬러가 있는데 구글은 왜 없냐" 질문 — 원인은 설계
의도가 아니라 **누락**이었음: 구글 버튼은 38번 항목(2026-07-20, 인증 3종 세트 최초
구현)에서 만든 자리표시자 스타일(테두리만 있는 흰 버튼 + 아무 색 없는 "G" 글자)이 그대로
남아있었던 것이고, 그 뒤 134/142번에서 네이버·카카오를 추가할 때는 각 사의 실제 공식
버튼 가이드(네이버 초록 배경, 카카오 노란 배경)를 따라 만들면서 구글 버튼은 다시 손대지
않아 생긴 불일치.

- 참고로 구글은 네이버·카카오와 달리 "단색 배경" 공식 가이드가 없고, 흰 배경 + 컬러풀한
  "G" 로고 마크가 공식 "Sign in with Google" 버튼 스타일임 — 그래서 배경을 특정 파란색
  등으로 칠하는 대신, 실제 구글 4색 G 로고(SVG)를 넣는 방향으로 수정.
- `app/src/app/login/page.tsx`: 구글 버튼에 공식 4색 G 로고 SVG(노랑/빨강/초록/파랑)를
  추가, 배경은 흰색 유지(`bg-white`), 텍스트 `text-gray-700`로 소폭 조정.
  네이버(#03C75A 배경)·카카오(#FEE500 배경) 버튼은 각 사 공식 스타일이라 변경 없음.
- 검증: `npx tsc --noEmit` 클린. 시각적 변경이라 재홍님 로컬 확인 권장.
