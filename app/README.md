...
app/
  ...
  src/auth.ts / auth.config.ts   NextAuth 설정 (이메일/비밀번호 + 구글/네이버/카카오 소셜 로그인)
...
- **소셜 로그인**: 구글 + 네이버 + 카카오를 지원합니다(네이버·카카오는 2026-08-07 추가). 카카오 OAuth 로그인은 2026-07-22 한 번 삭제됐다가(`KAKAO_REST_API_KEY`는 로그인용이 아니라 골프장 좌표 지오코딩의 주소 검색용 별개 키입니다) 2026-08-07 다시 요청받아 재추가했습니다. 네이버는 `NAVER_CLIENT_ID`/`NAVER_CLIENT_SECRET`, 카카오는 `KAKAO_CLIENT_ID`/`KAKAO_CLIENT_SECRET`를 각각 개발자센터에서 발급받아 `.env`에 입력해야 실제 로그인이 동작합니다(콜백 URL: `/api/auth/callback/naver`, `/api/auth/callback/kakao`).
