...
- **현재 상태**: 화면 14개 설계 완료. **사용자 화면 10개(1~10번)는 전부 실구현 완료** — 8번(스코어 조회 목록)까지 이번 세션에 마무리(75번 항목). 관리자 화면 4개(11~14번)는 설계만 되어있고 구현 착수 전인 것이 유일하게 남은 화면 단위 작업.
- **미완료(다음에 바로 할 일 후보)**:
  1. **관리자 화면 4개 실구현 착수** — 골프장 Par 관리 목록/입력/CSV 업로드/회원 관리. 목업·설계 문서는 이미 루프 UI 기준으로 완료돼 있음(15번 항목).
  2. 기상청 날씨 API 연동 미착수(`Round.weatherSnapshot` 항상 null 상태로 방치).
  3. `app/README.md`·루트 `README.md`가 구버전 기준으로 멈춰 있음 — 스키마/화면 갱신 내용과 동기화 필요.
  4. 로컬 DB 접근 불가 환경 제약(재홍님과 논의 완료, 2026-07-22): 이 샌드박스는 재홍님 PC와 분리된 격리 환경이라 `localhost:5432`(Docker든 네이티브 설치든 무관)에 도달 불가, 아웃바운드도 극히 제한적 allowlist(github.com 외 대부분 차단 확인)라 클라우드 DB로 옮겨도 당장은 안 됨 — 실제 DB 동작 검증은 계속 재홍님이 로컬에서 진행하는 방식 유지하기로 확정.
...
- 세부 이력은 아래 8~75번 항목(시간순 작업 로그)에 모두 기록되어 있음.
