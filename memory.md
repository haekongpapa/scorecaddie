...
- **현재 상태**: 화면 14개 설계 완료. 실제 Next.js 구현은 1~14번 사용자 화면 대부분 완료 상태(4 대시보드/5 골프장목록/6 골프장상세/7-1·7-2 스코어등록/9 라운드상세 모두 실구현), **8번(스코어 조회 목록)만 여전히 placeholder로 남은 마지막 미구현 사용자 화면**. 관리자 화면 4개는 설계만 되어있고 구현 착수 전.
- **미완료(다음에 바로 할 일 후보)**:
  1. **8번 스코어 조회 목록 실구현** — 남은 유일한 사용자 화면. 기간/골프장 필터, 내 기록만/전체 회원(thirdPartyConsent 동의자만) 조건 반영 필요.
  2. 관리자 화면 4개(골프장 Par 관리/CSV 업로드/회원관리) 실구현 착수 전.
  3. `app/README.md`·루트 `README.md`가 구버전 기준으로 멈춰 있음 — 스키마/화면 갱신 내용과 동기화 필요.
  4. 기상청 날씨 API 연동 미착수(`Round.weatherSnapshot` 항상 null).
- **작업 시 필수 규칙**: 이 프로젝트 폴더(`ScoreCaddie`)에 대한 파일 쓰기는 `Edit`/`Write` 툴이 아니라 항상 `mcp__workspace__bash`의 heredoc(`cat > file << 'EOF' ... EOF`)으로 하고, 직후 `wc -c`/`tail`/`grep -cP '\x00'`로 검증할 것(파일이 조용히 잘리거나 변경이 아예 반영 안 되는 마운트 버그가 반복 확인됨 — 8~12번 항목 참고). memory.md처럼 큰 파일은 python3(`open().write()`)로 전체 치환하는 것도 안전(순수 파일 I/O라 Edit/Write 툴의 truncation 버그와 무관).
- 세부 이력은 아래 8~74번 항목(시간순 작업 로그)에 모두 기록되어 있음.
