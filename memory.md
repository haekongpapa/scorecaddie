...
- **화면 단위 밖에서 남은 작업(우선순위 순서 없음, 사용자 지정 대기)**:
  1. ~~`app/README.md`·루트 `README.md`가 구버전 기준으로 멈춰 있음~~ — 둘 다 123번 항목(2026-07-29)까지 전면 재작성 완료. 더 이상 대기 항목 아님.
  2. ~~카카오 OAuth 로그인~~ — **2026-07-22 사용자 요청으로 기능 자체 삭제(86번)**. 더 이상 대기 항목 아님. 로그인은 구글 OAuth + 이메일/비밀번호만 지원.
...
- [x] ~~`app/README.md` / 루트 `README.md` 동기화~~ — 루트 `README.md`는 121번 항목(2026-07-29)에서 재작성 완료. `app/README.md`는 여전히 미착수.
...
**후속 확인(2026-07-22, 같은 날)**: 사용자가 로컬에서 `npx prisma migrate deploy` + `npx prisma generate`, `git push` 완료 확인. **남은 건 11번(골프장 Par 관리) "골프장 공공 데이터 업로드" 재실행(addressLotno 백필) → "좌표 지오코딩 실행" 재실행** 두 가지(둘 다 화면 버튼 클릭, 사용자 진행 예정).

### 다음 세션 시작 시

- 위 남은 2단계(공공 데이터 재동기화 → 지오코딩 재실행) 결과를 사용자에게 확인. 여전히 실패하는 골프장이 있으면 사유(errors 배열) 확인 후 추가 대응. 남은 화면 밖 작업(README 동기화/카카오 로그인 테스트/하이원CC 자료)은 계속 사용자 지정 대기.

## 83. 지오코딩 잔여 실패 9건 진단 — 에러 응답에 실제 조회 주소값 포함 (2026-07-22)

82번 적용 후 재실행 결과 9개 골프장이 여전히 실패(대부분 "주소 검색 결과 없음 + 키워드 검색도 결과 없음", 1건은 도로명·지번 둘 다 시도 후 실패). 샌드박스에서 DB에 직접 접근할 수 없어 이 9건의 실제 `address`/`addressLotno` 값을 볼 수 없는 상태 — 원인 분석을 위해 `api/admin/golf-courses/geocode/route.ts`의 `errors` 배열에 `address`/`addressLotno` 필드를 추가해, 사용자가 결과 JSON을 공유하면 DB 접근 없이도 바로 어떤 문자열이 카카오로 전달됐는지 확인 가능하게 함. `npx tsc --noEmit` EXIT_CODE=0 확인.

### 다음 세션 시작 시

- 사용자가 11번(골프장 Par 관리) "좌표 지오코딩 실행" 재실행 후 남은 9건의 에러 JSON(이번엔 address/addressLotno 포함)을 공유하면, 실제 주소 문자열을 보고 카카오가 못 찾는 이유(비정형 지번/괄호 표기/시설명 자체 미등록 등)를 구체적으로 진단하고 대응할 것.

## 84. 잔여 실패 9건 원인 확정 + 주소 정규화 추가 (2026-07-22)

83번에서 추가한 address/addressLotno 노출 덕에 실제 실패 사유 확정. 웹 검색으로 교차 확인한 결과 두 가지 패턴으로 정리됨:

- **"산 5" 공백 문제(9건 중 7건)**: 지번주소 표기법상 "산"은 번지 앞 접두사라 공백 없이 "산5-1"로 붙는 게 정식 표기인데, 공공데이터 원본엔 "산 5"처럼 공백이 섞여 옴. 카카오 주소 검색이 이 공백을 파싱 못 하는 것으로 판단(경주신라CC/광산 친환경대중골프장/대중제 골프장/백암비스타/오크밸리대중골프장/제이드팰리스솔프클럽/휘닉스파크골프장, 총 7건 모두 이 패턴).
- **"전남광주통합특별시" 신설 지명 문제(2건: 광산 친환경대중골프장, 여수아일랜드)**: 웹 검색으로 확인 — **2026-07-01 광주광역시+전라남도 통합으로 실제 신설된 광역자치단체명**(데이터 오류가 아니라 진짜 현재 행정구역명). 공공데이터는 이미 새 지명으로 갱신됐지만, 카카오 로컬 API 주소 인덱스는 아직(2026-07-22 기준) 이 신설 지명을 반영 못 한 상태라 검색이 안 됨 — 지도 서비스 쪽 데이터 갱신 지연이 원인.
- **"(재)한국군사문제연구원남성대쌍둥이골프연습장"(경기도 성남시 수정구 창곡동 92-1)은 위 두 패턴 어디에도 해당 안 됨** — 원인 미상(실내 연습장이라 카카오맵에 애초에 등록 자체가 안 돼 있을 가능성). 이번 수정으로도 해결 안 될 수 있음.

**수정**: `lib/geocoding/kakao.ts`에 `normalizeForKakao()` 추가 — 주소 검색(①②) 단계에 실제 질의 직전 적용. ① `산\s+(\d)` → `산$1`(공백 제거). ② `전남광주통합특별시` 접두사는 카카오가 아직 인식하는 옛 지명(광주 5개 구는 "광주광역시", 그 외는 "전라남도")으로 치환. 키워드(장소명) 검색 단계는 지명 문제와 무관하므로 정규화 미적용. `shouldTryLotno`(address vs addressLotno 중복 판단)는 정규화 전 원본 값 기준 유지(로직 변경 없음). Node 스크립트로 9개 실패 주소 전체에 정규화 함수를 직접 실행해 의도대로 변환되는지 확인(7건 "산" 공백 제거, 2건 지명 치환, 1건 무변화 확인). `npx tsc --noEmit` EXIT_CODE=0.

※ 카카오가 향후 "전남광주통합특별시" 자체를 인덱싱하면 이 치환은 불필요해짐 — 다음에 광주/전남 관련 지오코딩 실패가 재발하면 이 부분부터 의심할 것.

### 다음 세션 시작 시

- 사용자가 로컬에서 코드 반영(스키마 변경 없음, 서버 재시작만 필요) 후 "좌표 지오코딩 실행" 재실행 → 9건 중 몇 건이 해결됐는지, "성남시 수정구 창곡동 92-1"(한국군사문제연구원 연습장)이 여전히 실패하는지 확인. 계속 실패하면 수동 좌표 입력 등 대안 논의 필요(현재 관리자 화면엔 수동 좌표 입력 UI 없음).

## 85. 84번 정규화 가설 두 개 모두 기각 — 실제 원인은 데이터 소스 간 지번 불일치 (2026-07-22)

84번에서 세운 두 가설("산 5" 공백 문제 / "전남광주통합특별시" 신지명 미반영)을 사용자가 map.kakao.com에서 직접 검색해 반증함:

- "경상북도 경주시 신평동 산 5"(공백 있음) 검색 → 카카오가 **"산 5-2"(경주신라컨트리클럽)** 라는 "유사 주소"를 제안. 공백 없앤 "산5"도 결과 동일 — **공백은 원인이 아니었음**.
- "전남광주통합특별시 광산구 연산동 산 118번지" 검색 → 카카오가 **"전남광주통합특별시 광산구 연산동 산 113-4"** 라는 유사 주소를 제안 — **카카오는 이미 "전남광주통합특별시" 지명을 정상 인식하고 있었음**(84번 가설과 반대). 지명 문제가 아니었음.

**진짜 원인**: 두 사례 모두 카카오 지도에 등록된 실제 지번(산 5-2, 산 113-4)이 공공데이터(LOTNO_ADDR)의 지번(산 5, 산 118번지)과 **다름** — 데이터 소스 간(공공데이터 vs 카카오맵) 지번 정보 자체의 불일치. 카카오 웹사이트 검색은 "유사 주소" 퍼지 매칭/제안 기능이 있어 이런 경우도 사용자에게 대안을 보여주지만, `search/address.json` REST API는 정확히 일치해야 결과를 반환하고 유사 주소 제안 기능이 없어 그대로 실패함.

**결론(사용자 지시)**: 84번에서 추가한 `normalizeForKakao()`(산 공백 제거, 전남광주통합특별시 치환)는 **틀린 가설에 기반한 코드라 제거**. `lib/geocoding/kakao.ts`는 82번 상태(address→addressLotno→키워드 3단계 폴백, 정규화 없음)로 되돌림. 이 데이터 불일치 문제는 코드로 해결 가능한 종류가 아니므로 **그대로 유지하기로 확정** — 잔여 9건은 필요 시 사용자가 카카오맵에서 정확한 지번을 확인해 수동으로 좌표를 넣는 방법 정도만 남음(현재 수동 좌표 입력 UI는 없음, 향후 요청 시 추가 검토).

**검증**: `npx tsc --noEmit` EXIT_CODE=0.

### 다음 세션 시작 시

- 좌표 지오코딩 관련 잔여 9건은 데이터 소스 불일치로 결론짓고 종료. 추가로 코드 차원의 정규화 시도는 하지 않음(재발해도 이 85번 항목부터 확인). 남은 화면 밖 작업(README 동기화/카카오 로그인 테스트/하이원CC 자료)은 계속 사용자 지정 대기.

## 86. 카카오 계정 로그인 기능 삭제 (2026-07-22)

지오코딩 작업 마무리 후 사용자가 카카오 로그인 설정 방법을 물어봐서 안내했으나(Kakao Developers 콘솔 설정 + 이메일 동의항목이 `User.email` 필수 제약과 충돌 가능성 있다는 점까지 설명), 곧이어 "카카오 계정 로그인 기능은 삭제해줘"로 결정 변경 — 콘솔 설정 안내 대신 기능 자체를 제거.

- `auth.config.ts`: `Kakao` provider import 및 등록 제거(Google provider만 남음).
- `auth.ts`: 주석에서 "구글/카카오" → "구글"로 정정.
- `app/login/page.tsx`: "카카오로 계속하기" 버튼 제거(구글 버튼만 유지).
- `.env`: `KAKAO_CLIENT_ID`/`KAKAO_CLIENT_SECRET` 두 줄 삭제. **`KAKAO_REST_API_KEY`(골프장 좌표 지오코딩용, 79~85번)는 완전히 별개 기능이라 그대로 유지** — 실수로 같이 지우지 않도록 주의.
- `npx tsc --noEmit` EXIT_CODE=0.
- `doc/개발리스트.md` "카카오 OAuth 로그인" 행을 "삭제됨"으로 갱신.
...
## 88. 배포 방안 최종 결정 슬라이드 추가 — Vercel + Supabase로 변경 (2026-07-27)

87번에서 만든 PPT의 마지막 "PM 추천안"(Vercel+Neon) 슬라이드에 대해, 사용자가 "Neon의 유휴 시 콜드스타트가 걸려서 Supabase로 하고 싶다"는 의견 제시 → Neon 콜드스타트 실측치(2026년 기준 약 0.5~1.8초, WebSearch로 재확인)를 공유하며 논의한 뒤, 사용자가 Vercel + Supabase로 최종 결정.

- **PPT 변경**: 기존 12번 슬라이드(PM 추천안: Vercel+Neon)는 그대로 두고, **13번 슬라이드 "최종 결정"을 신규 추가** — Vercel+Supabase 확정, 근거 4가지(콜드스타트 없음/정액 요금 예측 가능/Auth·Storage·Realtime 번들 활용 가능/Neon 장점은 초기 단계에 불필요), 다음 단계 3가지(예산·도메인 확정 → **Supabase** 프로젝트 생성 → Vercel 연동)로 구성. `gen_deploy_ppt.js`(outputs 스크립트)에 슬라이드 13 블록을 추가하고 전체 재생성하는 방식으로 작업(기존 슬라이드 XML을 직접 수정하는 대신, 원본 생성 스크립트를 갖고 있어 처음부터 다시 만드는 쪽이 더 안전하다고 판단).
- **환경 이슈 발견**: `soffice.py --convert-to pdf`가 한글 파일명(`ScoreCaddie_배포방안_검토.pptx`)에 대해 `SfxBaseModel::impl_store ... Abort Code:27`로 계속 실패 — ASCII 파일명(`deploy_review.pptx`)으로 복사해서 변환하니 정상 동작. 향후 이 프로젝트에서 pptx를 LibreOffice로 PDF 변환/QA할 때 한글 파일명이면 이 우회가 필요할 수 있음(참고용으로 기록).
- **파일 저장 이슈(중요)**: 이미 `doc/ScoreCaddie_배포방안_검토.pptx`로 저장돼 있던 이전 버전을 덮어쓰려 했으나 `cp`가 `Permission denied`로 실패 — `mcp__cowork__allow_cowork_file_delete`로 덮어쓰기 허용을 요청했으나 **사용자가 거부**. 결과적으로 기존 파일은 그대로 두고 새 버전을 `doc/ScoreCaddie_배포방안_검토_v2.pptx`(13슬라이드, 최종 결정 포함)로 별도 저장 — **현재 doc 폴더에 두 버전이 공존**(v1=12슬라이드/PM 추천안까지, v2=13슬라이드/최종 결정 포함). 사용자가 필요 시 v1을 직접 정리(삭제 또는 이름 변경)해야 함 — 나는 프로젝트 폴더 내 기존 파일을 지우거나 덮어쓸 권한이 없음.

### 다음 세션 시작 시

- 사용자에게 `doc/` 폴더에 배포방안 검토 PPT가 두 벌(v1, v2) 남아있다는 점 상기 — 필요하면 v1 삭제 여부를 다시 확인하거나, v2를 원래 파일명으로 정리하는 방법(사용자가 로컬에서 직접 파일 교체)을 안내할 것. 이후 다음 단계(배포 예산·도메인 확정 → Supabase 프로젝트 생성 → Vercel 연동)를 실제로 착수할지는 사용자 지정 대기.


## 89. 디렉터리 구조 · 코딩 스타일 가이드 문서 신설 (2026-07-27)

사용자가 Express/레이어드 아키텍처 기준의 디렉터리 규칙(controllers/routes/models/services/middlewares 등)과 코딩 스타일 규칙(타입/모듈화/에러처리/로깅/테스트) 초안을 제시 → "바로 적용하지 말고 검토해서 ScoreCaddie에 맞게 의견을 제시해달라"는 요청에 따라 PM이 항목별로 리뷰한 뒤, 사용자가 "정리해서 가이드 문서로 만들어달라"고 확정 요청.

- **리뷰 핵심**: ScoreCaddie는 Express를 쓰지 않고 Next.js App Router로 프론트/백엔드를 통합 운영 중이라, `routes/`·`controllers/`는 App Router의 파일 기반 라우팅이 대신하고, `models/entities/`는 `prisma/schema.prisma`가 이미 그 역할을 함 — 세 항목 모두 도입하지 않는 쪽으로 정리. `services/`는 기존 `src/lib/` 구조와 동일 개념이라 이름만 유지, `middlewares/`는 Next.js가 루트 `middleware.ts` 단일 파일 구조라 폴더화 불가. `config/`(env 접근 창구)는 지금 없는 상태라 신규 도입 권장.
- **신규 파일**: `doc/coding-guidelines.md` — 위 리뷰를 반영한 최종 가이드. 디렉터리 구조 원안 대비 변경사항 표, TypeScript 규칙(any 지양 + `tsconfig strict:false` 유지 배경 + 판별 유니언은 `"필드명" in 값` 방식으로 설계하라는 80번 항목의 교훈을 프로젝트 규칙으로 명문화), 코딩 스타일(모듈화/API Route는 얇게+로직은 lib로 위임/에러처리는 fail-soft 패턴+공통 래퍼/로깅은 winston 대신 console 레벨 구분 — Vercel 서버리스 환경 이유 포함), 테스트(Vitest+Playwright 권장, 아직 착수 전이라 규칙만 선제 확정) 순으로 구성.
- **성격**: 고정 규칙이 아니라 프로젝트가 커지면서 갱신 가능한 살아있는 문서로 문서 자체에 명시.
...
- 2026-07-28: 사용자가 4단계(공공데이터 동기화/CSV 업로드/지오코딩) 로컬 검증 및 git push
  완료 확인.


## 95. coding-guidelines.md 적용 5단계 — 죽은 파일 정리 (2026-07-28)

**5단계**: `lib/geocoding/_repro.ts`(2026-07-22에 "삭제 시도했으나 마운트 delete 권한 문제로
대신 비워둠"이라는 주석만 남아있던 빈 파일) 정리.

- 샌드박스 마운트에서는 이번에도 `rm`/`git rm` 모두 "Operation not permitted"로 막혀 삭제 불가
  (기존에 겪던 것과 동일한 제약). 사용자가 로컬 탐색기에서 직접 삭제 후 요청해 커밋 진행.
- 참조하는 곳 없음(`grep -rn "_repro"` 결과 0건) 확인 후 삭제였으므로 회귀 위험 없음.
...
## 108. "User 테이블에 데이터 없음" — 순서 문제, 가이드 단계 순서 수정 (2026-07-28)

재홍님이 pgAdmin으로 `User` 테이블을 열어보니 데이터가 없다고 보고. **정상 동작임** —
마이그레이션은 스키마(테이블 구조)만 만들 뿐 데이터는 비어있는 게 당연하고, 회원가입/로그인을
한 번도 안 했으니 `User` 행이 없는 것. 가이드가 "4.관리자 지정 → 5.스모크 테스트" 순서로
되어 있어 관리자 지정을 먼저 하려다 빈 테이블을 마주친 것으로 보임 — **순서가 잘못 안내된 게
원인**.

- **가이드 수정**: `doc/supabase-deploy-guide.md` 4/5절 순서를 뒤바꿔 "4.스모크 테스트(회원가입으로
  User 행 생성이 먼저) → 5.관리자 지정(생성된 행을 확인하고 role 수정)"으로 재배치. 체크리스트도
  지금까지 완료된 항목(프로젝트 생성~pgAdmin 연결까지)을 `[x]`로 갱신.
...
## 119. Vercel 배포 마일스톤 완료 (2026-07-29)

118번의 next 15.1.12 패치 후 재홍님이 push → Vercel 재배포 성공. **배포 도메인:
https://scorecaddie.vercel.app/** — WebFetch로 랜딩 페이지 정상 렌더링 확인.

이어서 4단계(Google OAuth 프로덕션 리다이렉트 URI/원본 등록) 진행 후, 재홍님이 스모크
테스트 전 항목 직접 확인 완료: 이메일/구글 로그인, 골프장 목록 조회, 스코어 등록·조회·라운드
상세, 관리자 화면 진입. **"Vercel에 앱 배포 진행" 요청(2026-07-29 세션 시작 시점) 마일스톤
완전 종료.**

- `doc/vercel-deploy-guide.md`에 7번 섹션(배포 완료 요약, 도메인, CVE 대응 이력)과 체크리스트
  전체 완료 표시 추가.
- `doc/개발리스트.md` 8번(테스트 → 배포)을 완료로 갱신, Vercel 가이드 문서 링크 추가.
- 남은 것은 (선택) 커스텀 도메인 연결뿐 — 예산/도메인 보유 여부 미정 사항(275번 줄)으로
  그대로 대기.

### 다음 세션 시작 시

- ScoreCaddie MVP 개발·테스트·배포 전체 사이클(기획 → 화면/API 구현 → Supabase DB →
  Playwright e2e 8개 시나리오 → Vercel 배포) 1주기 완전 종료. 다음 세션은 재홍님이 새로
  지시하는 기능 추가/버그 수정/커스텀 도메인 등으로 이어짐 — 특별히 대기 중인 후속 과제 없음.
...
## 121. 분석설계 요약 PPT v5 전면 갱신 + 화면 md 문서 동기화 (2026-07-29)

120번(기능 수정 3건) 커밋 후 재홍님이 "이번에 추가한 작업 관련 문서에 업데이트 해줘 — 분석설계
파일 및 화면 md 파일 등등"으로 요청. 확인해보니 `doc/ScoreCaddie_분석설계_요약.pptx`가 v4
(2026-07-16)에서 멈춰 있어 그 이후 2주간의 작업(관리자 화면 4개, Supabase 연결, Playwright
e2e 8개 시나리오, Vercel 배포, 오늘 작업 등)이 전혀 반영 안 된 상태였음 — 이번 세션 범위만
반영할지, 전체를 현재 상태로 따라잡을지 AskUserQuestion으로 확인 → **"v5로 전체 갱신"** 선택.

**`doc/ScoreCaddie_분석설계_요약.pptx` v5 (18슬라이드, 전면 재작성)**:
- 기존 v4는 XML 부분 편집이 아니라 **pptxgenjs로 처음부터 새로 생성**하는 방식을 택함(거의
  모든 슬라이드의 수치·사실관계가 바뀌어야 해서 부분 편집보다 안전하다고 판단).
- 팔레트 신규 확정: "Fairway"(포레스트그린 `1B4332` 주색 + 세이지 `6B9080` 보조 + 골드
  `C99A3E` 강조) — 골프 서비스 톤에 맞춘 선택, 기존 v4의 색 구성과는 다름(전면 리디자인).
- 신규 슬라이드 4개: 관리자 화면 4종(11~14번) 구현 현황 / 골프장 공공데이터 연동·좌표
  지오코딩 상세 / 테스트&배포(Playwright e2e 8개 + Vercel) / **오늘의 기능 개선(2026-07-29)
  3건 전용 슬라이드**(스코어 등록 콤보박스, 공공데이터 증분동기화, 로딩 차단).
- 기존 슬라이드 전부 사실관계 갱신: 핵심기능 8개(카카오 로그인 삭제 반영), DB 7개 핵심
  테이블(GolfCourseSyncLog 추가), DB 컬럼 정의(addressLotno/startTime/GolfCourseSyncLog
  필드 추가), 기술스택(Supabase 확정, 로컬 DB도 Supabase 공유로 전환 반영, Vercel 배포
  URL), 정책 결정사항(로그인/데이터소스/관리자권한/배포스택 전부 갱신), 구현 진행 현황(테스트·
  배포 항목 "진행예정"→"완료"로 전환), 주요 이슈 4건(Supabase IPv6 이슈·Next.js CVE 대응
  추가), 다음 논의 사항(라운드상세 UI 확정 항목 제거 - 이미 해결됨, 커스텀 도메인/모바일
  대응 등 갱신).
- **시각 QA 중 발견한 문제**: 초안에 이모지(🚀✅🗄👤⛳ 등)를 아이콘으로 썼더니 LibreOffice
  렌더링에서 단순히 tofu 박스로 안 뜨는 정도가 아니라 **인접 한글 텍스트 일부가 깨지는
  문제**를 발견("공공데이터"가 "?공데이터"로 렌더링되는 등) — heredoc으로 멀티바이트 이모지를
  JS 문자열 리터럴에 넣는 과정에서 인코딩이 꼬인 것으로 추정. 전부 제거하고 기존 슬라이드에서
  이미 쓰던 숫자/글자 배지(원형 도형 + 텍스트) 패턴으로 통일 — 이후 재검증 결과 깨짐 없음.
  **교훈**: 이 환경(bash heredoc → pptxgenjs)에서 pptx 생성 시 이모지 유니코드는 피하고
  숫자/영문 약자 배지로 대체할 것.
- 검증: `markitdown` 콘텐츠 확인(플레이스홀더/오타 없음), `validate.py` PASS, LibreOffice
  변환 후 18장 전체 육안 검수(오버플로우/겹침/저대비 없음, 위 이모지 이슈 수정 후 재검수 통과).

**`doc/pages.md` 갱신**:
- 2번(로그인) 화면 설명에 카카오 소셜 로그인이 여전히 남아있던 것을 발견해 정정(2026-07-22에
  이미 삭제된 기능인데 문서만 안 고쳐져 있었음) — 목적/컴포넌트/데이터 의존성 3곳 전부 수정 +
  삭제 이력 각주 추가.
- 7-1(스코어 등록 Step1)에 콤보박스 전환, 골프장 조회 조건(루프+영업상태), `initialCourseId`
  안내 문구 반영(120번 작업 내용).

**`doc/개발리스트.md` 갱신**: 7-1/11번 섹션에 120번 작업(콤보박스, "전체" 옵션, 증분 동기화,
로딩 오버레이, initialCourseUnavailable 버그 수정) 반영.

**커밋하지 않은 것**: `to-do.txt`가 작업 중 로컬에서 재홍님이 직접 수정한 상태로 남아있었는데
(DB/npm 실행 명령 메모 갱신 + 맨 끝에 `git.northstar.co.kr` URL과 자격증명으로 보이는 줄
`wi97 / Hunter@!99` 추가됨) — 이번 작업과 무관하고 자격증명으로 보이는 내용을 실수로 커밋하면
안 되므로 git add에서 의도적으로 제외함. 재홍님이 그 파일의 마지막 줄이 실제로 커밋하면 안 되는
자격증명인지 한 번 확인해보시길 권장(민감정보라면 `.gitignore`에 추가하는 것도 고려).

### 다음 세션 시작 시

- 문서 동기화 작업 완료. `to-do.txt` 마지막 줄(자격증명으로 보이는 내용) 커밋 여부/민감정보
  처리 필요한지 재홍님 확인 필요 — 확인 전까지는 계속 git add에서 제외할 것.
- `app/README.md`/루트 `README.md` 동기화는 여전히 미착수 상태로 남아있음(0번 가이드에
  오래전부터 대기 중인 항목) — 다음에 문서 정리를 다시 요청받으면 이것도 후보로 고려.


## 122. 루트 README.md 재작성 — 개요/분석설계/구현/배포 4단계 요약 (2026-07-29)

121번(분석설계 PPT v5) 직후 재홍님이 "README.md 파일을 지금까지 진행한 프로젝트 내용을
요약해서 다시 만들어줘. 프로젝트 개요, 분석/설계, 구현, 배포 단계로 요약하고 각 단계 별
산출물 파일도 표시해줘. 산출물은 ppt, md 파일이면 될것 같아"로 요청.

- 기존 루트 README.md는 2026-07-15 초기 세팅 당시 내용(VS Code 연동/빠른 시작/GitHub 연동
  안내) 그대로 멈춰 있었음 — "0. 다음 세션 시작 가이드" 1번 항목에 오래 대기 중이던 문제.
- 4단계(개요/분석·설계/구현/배포)로 전면 재작성, 각 단계에 실제 `doc/` 산출물 파일을
  표로 연결. PPT는 여러 단계에 걸쳐 관련 슬라이드가 있는 경우(`ScoreCaddie_분석설계_요약.pptx`
  등) 해당 단계마다 어느 슬라이드 구간인지 명시.
- 기존 "빠른 시작"/"디렉터리 구조" 절은 유지하되 로컬 DB를 Supabase로 전환한 현재 상태에
  맞게(Docker Compose 실행 안내 제거, Supabase 공유 DB 안내로 교체) 갱신.
- `to-do.txt`: 재홍님이 로컬에서 자격증명으로 보이던 마지막 줄(git.northstar.co.kr URL +
  계정정보)을 직접 삭제 확인 → 이번엔 안전하게 커밋함(cd 경로 보완/pgAdmin 실행법/DB 스키마
  변경 절차 메모도 함께 반영).
- memory.md "0. 다음 세션 시작 가이드" 1번 항목과 "5. 미정 사항"의 README 동기화 항목을
  완료로 갱신 — `app/README.md`는 여전히 미착수 상태로 남겨둠(다음 후보).

### 다음 세션 시작 시

- 특별히 대기 중인 후속 과제 없음. `app/README.md` 동기화가 다음에 문서 정리를 요청받으면
  가장 유력한 후보.


## 123. app/README.md 재작성 + .env DATABASE_URL 로컬 Docker로 되돌아가 있던 것 발견·복구 (2026-07-29)

122번(루트 README.md) 직후 재홍님이 "app/README.md 파일도 정리해줘"로 요청.

- **작업 중 발견**: `app/README.md`를 다시 쓰기 전 `.env`를 확인하니 `DATABASE_URL`이
  Supabase가 아니라 `localhost:5432`(로컬 Docker Postgres)를 가리키고 있고, Supabase 줄은
  주석 처리되어 있었음 — 105번 항목에서 Supabase로 전환한 이후 로컬 Docker는 안 쓰는 것으로
  알고 있었는데 실제로는 되돌아가 있어서, 루트 README.md(122번)에 "로컬도 Supabase 공유"라고
  이미 적어놓은 내용과 어긋날 위험이 있어 AskUserQuestion으로 확인 → 재홍님이 "내가 supabase로
  수정했음"이라고 직접 `.env`를 고쳐서 확정. (원인은 불명 — 재홍님이 다른 용도로 잠깐
  로컬 Docker를 썼다가 되돌리는 걸 잊었을 가능성이 높아 보이나 확인은 안 됨.)
- **`app/README.md` 전면 재작성**: 2026-07-15 최초 스캐폴딩 시점 내용(Docker Compose 로컬
  DB 실행 안내, 8개 테이블 스키마, 카카오 OAuth 언급, node_modules 삭제 경고, "다음 구현
  단계(미구현)" 목록 등)이 전혀 안 고쳐진 채 남아있던 것을 확인하고 현재 상태 기준으로 새로
  작성:
  - 폴더 구조(7개 핵심 테이블+NextAuth 3, 마이그레이션 9건, e2e/ 포함)
  - 실행 순서를 Supabase 공유 DB 전제로 재작성(Docker 기동 단계 삭제)
  - 스키마 변경 시 `migrate dev` 안내, 단위/e2e 테스트 실행법(`npm run test`/`test:e2e`,
    dev 서버 기존 실행 종료 필요 경고 포함 — 116번 항목 교훈)
  - 핵심 참고사항(관리자 계정 수동 지정, 카카오 로그인 삭제, 골프장 데이터/루프 구조)
  - 관련 문서 링크(pages.md/분석설계 PPT/배포 가이드/coding-guidelines/memory.md)
- memory.md "0. 다음 세션 시작 가이드" 1번 항목을 완전 완료로 갱신(루트+app README 둘 다
  재작성 끝).

### 다음 세션 시작 시

- README 문서 정리 완전 종료. 특별히 대기 중인 후속 과제 없음.

## 124. 11번 화면 명칭 변경 — "골프장 Par 관리" → "골프장 관리" (2026-07-30)

재홍님 질문: `"골프장 Par 관리" 화면 명칭을 "골프장 코스 관리"로 변경하는게 어때?`

- **의견 제시**: 방향(현재 이름이 화면 실제 범위—검색/필터, 공공데이터 동기화, 좌표 지오코딩,
  CSV/루프 편집 진입점—에 비해 "Par 관리"로 너무 좁게 들림)에는 동의했지만, 제안된 "코스 관리"는
  이 앱에서 "코스"가 이미 루프(전반/후반코스)를 가리키는 용어로 쓰이고 있어(`RoundStep1.tsx`,
  `doc/pages.md`) 12번 화면("루프·Par 입력")과 역할이 헷갈릴 위험이 있다고 지적. 대안으로
  "골프장 관리"/"골프장 정보 관리"를 제시 → AskUserQuestion으로 확인, 재홍님이 **"골프장 관리"**
  선택.
- **반영 범위** (전체 "골프장 Par 관리" 문자열을 "골프장 관리"로 치환):
  - 소스코드 5곳: `app/src/app/admin/golf-courses/page.tsx`(TopBar title),
    `app/src/app/dashboard/page.tsx`(관리자 배너), `app/src/lib/geocoding/kakao.ts`(주석),
    `app/e2e/admin-geocode.spec.ts`·`app/e2e/admin-sync.spec.ts`(heading assertion)
  - 문서 6곳: `doc/admin-golfcourse-sync.md`, `doc/pages.md`(3곳), `doc/supabase-deploy-guide.md`,
    `doc/개발리스트.md`(2곳), 루트 `README.md`
  - `doc/ScoreCaddie_분석설계_요약.pptx`: python-pptx로 텍스트 런 2곳(슬라이드 3, 11) 직접 치환
    (deck 전체 재생성 아님 — pptxgenjs 재실행 대신 기존 파일의 run.text만 교체해 다른 슬라이드
    영향 없이 안전하게 수정).
  - `memory.md`: 8번 항목("사용자 커뮤니케이션 선호") 표기 예시를 "11번(골프장 관리)"로 갱신.
    단, 39/40/68/116/117/186/390/754/756/781/834/978/1210~1269번 등 **과거 로그 항목은
    당시 사실을 그대로 기록한 것이므로 소급 수정하지 않음** — 화면 명칭이 바뀐 히스토리 자체가
    기록 가치가 있다고 판단.
- `npx tsc --noEmit` EXIT_CODE=0 확인.

### 다음 세션 시작 시

- 화면 명칭 변경 완전 종료. 특별히 대기 중인 후속 과제 없음. 이후 "11번" 언급 시 "11번(골프장
  관리)"로 표기할 것(과거 항목 인용 시엔 그 항목이 쓰인 원문 그대로 "골프장 Par 관리" 유지).


## 125. 신규 기능 기획 — 골프장 주간 날씨 예보(+6일) & 골프 기록 분석 화면 (2026-07-30)

재홍님이 "기능 추가 - 날씨 정보 및 골프 기록 분석" 요청 → 코드 확인 결과 날씨는 이미
77/78/119번 등에서 실사용 검증까지 끝난 기능(단기예보 0~+3일, 스냅샷 저장, 목록/상세/대시보드
표시)이고, 분석은 대시보드 통계 카드 3개(총 라운드/평균타수/베스트)뿐이라는 걸 확인하고
AskUserQuestion으로 범위 확정:
- **날씨**: 기존 기능과 별개로, 골프장 선택 시 **오늘~+6일(7일치)** 예보를 보여주는 신규 위젯
- **분석**: **신규 전용 화면**, 지표는 스코어 추이·홀 유형별 상세·골프장별 비교·날씨 조건별
  비교 + "수집 중인 상세 데이터로 가능한 분석은 목업으로 제안해달라"

이어서 재홍님이 "바로 개발하지 말고 기획서를 만들어서 보여줘"로 요청 → **이번 세션은 구현
없이 기획 문서 + 목업까지만 진행**(개발 착수 전 단계).

**핵심 발견/설계 포인트**:
- 현재 날씨 코드(`lib/weather/kma.ts`)는 기상청 **단기예보**(`getVilageFcst`) 하나만 쓰고
  있고, 이 API는 구조적으로 오늘~+3일까지만 제공(`FORECAST_MAX_DAYS_AHEAD = 3`). +6일을
  보여주려면 기상청 **중기예보**(`getMidLandFcst` 육상예보 + `getMidTa` 기온, 공공데이터포털
  "기상청_중기예보 조회서비스")를 추가 연동해야 함 — D+4~D+6은 정보 밀도가 낮아짐(3시간
  간격 상세 대신 하루 오전/오후 텍스트 + 최고/최저기온만, 발표도 하루 2회뿐).
- 중기예보는 좌표/격자가 아니라 **예보구역코드(regId)**를 쓰므로 `GolfCourse.latitude/longitude`
  에서 바로 계산 불가 — 행정구역 기준 매핑표(공공데이터포털 "예보구역정보 조회서비스"
  활용 또는 정적 매핑표 내장)가 별도로 필요함을 확인. 정확한 regId 값/실제 스펙은 다음
  세션에 API 활용신청 후 확정 필요(단기예보 API도 최초엔 활용가이드만 보고 시작해 실사용
  중 보정한 전례가 있음 — 77/78번과 동일 패턴 예상).
- 분석 화면은 앱이 홀마다 이미 쌓고 있는 상세 필드(`teeShotResult`, 페널티/OB타수,
  `bunkerUsed`, `pinDistanceType/Meters`, `onGreenStrokes`/`puttStrokes`)를 근거로 페어웨이
  안착률·GIR(온그린타수 ≤ par-2 근사)·평균 퍼트수·Par유형별 평균스코어·핀거리별 퍼트수
  비교 등을 지표 후보로 제안 — 다른 화면에는 전혀 없는, 이 앱만의 차별화 지표.
- `doc/개발리스트.md`(192번 줄)에 이미 있던 사실 재확인: 프로덕션에 실제 라운드 데이터가
  거의 없어 대시보드 통계가 현재 0/-/-로 표시됨 — 분석 화면도 초기엔 빈 상태/표본부족 UX가
  중요하다는 점을 기획서에 명시.

**산출물**(전부 doc/, 코드 변경 없음):
- `doc/기능기획_날씨예보및기록분석.md` — 요구사항, 기술 제약(중기예보 필요성), 열린 질문
  목록, 다음 단계 정리
- `doc/mockups/06-2-weather-forecast.html` — 6번(골프장 상세) 화면에 7일 날씨 위젯을 추가한
  제안안(D0~D3 상세 카드 vs D4~D6 중기예보 카드를 점선 테두리로 시각적 구분)
- `doc/mockups/15-analysis.html` — 신규 "기록 분석" 화면 제안 목업. 상단 4탭(추이/골프장/홀
  상세/날씨) 전환형, 기존 `_shared.css`(`stat-grid`/`list-item`/`.seg` 패턴 등) 그대로 재사용,
  전부 샘플 데이터(실데이터 없음을 화면에 명시)
- `doc/mockups/index.html`에 위 두 목업을 "검토 중인 신규 제안(미확정)" 섹션으로 점선 카드
  추가(기존 14개 화면 갤러리는 그대로 유지, 총 개수 문구도 안 건드림 — 아직 정식 화면이
  아니므로)

**아직 안 건드린 것(의도적)**: `doc/pages.md`, `prisma/schema.prisma`, `doc/개발리스트.md`는
이번엔 수정하지 않음 — 기획서의 "열린 질문"(날씨 위젯 배치, regId 매핑 방식, 분석 화면
진입점 위치, 표본부족 기준 등)이 확정된 뒤 정식 반영 예정.

### 다음 세션 시작 시

- `doc/기능기획_날씨예보및기록분석.md`의 "열린 질문"(1.5절, 2.6절) 재홍님 확인 회신 대기.
- 확정되면 순서: `pages.md`/스키마 정식 반영 → 구현(날씨 위젯 → 분석 화면 순 제안, 순서
...
## 128. 로컬 검증 완료 — npm test 62/62, e2e 12/12 전체 통과 (2026-07-30)

127번 수정 반영 후 재홍님이 `npm run test:e2e` 전체 실행 결과 공유: **12개 시나리오 전부
통과**(`analysis.spec.ts` 포함 — 대시보드 "기록 분석" 카드 진입 + 4탭 전환 정상 확인).
이전에 공유받은 `npm test` 결과(127번 수정 후 재실행 시 62/62 통과 전제)까지 합쳐 15번
(기록 분석) 화면의 유닛/e2e 테스트 검증이 모두 끝남.

- `doc/개발리스트.md` 15번 섹션의 남은 🟡(e2e) 항목을 ✅로 갱신, 유닛테스트 항목도 실제
  로컬 실행 결과 반영 문구로 정리.
- 122~127번에 걸친 "15번 기록 분석 구현" 작업 단위가 완전히 종료됨: 기획(125) → 구현(126)
  → 환경 사고 복구 + 테스트 버그 수정(127) → 로컬 검증 완료(128).

### 다음 세션 시작 시

- 15번(기록 분석) 관련 대기 중인 후속 과제 없음.
- 날씨 예보(+6일, 06-2번)는 여전히 보류 — 재홍님이 재요청하면
  `doc/기능기획_날씨예보및기록분석.md` 1장부터 재개.
- `doc/ScoreCaddie_테스트계획서.pptx`에 9번째 시나리오(기록 분석)를 반영할지는 아직 미착수
  (급하지 않음, 문서 정리 요청 시 후보).


## 129. 기록 분석 추이 탭 막대 그래프 안 보이던 버그 수정 (2026-07-30)

재홍님이 "기록 분석 추이 화면에 막대가 표시 안됨" 보고.

- **원인**: `AnalysisTabs.tsx`의 최근 라운드 막대 차트에서, 부모 flex 컨테이너
  (`h-32`)에 `items-end`를 준 상태였음 — 이러면 각 열(자식)이 stretch되지 않고 내용
  높이만큼만 차지해, 정작 부모의 `h-32`(정해진 높이)가 그 자식에게 전달되지 않음. 그
  상태에서 막대 div에 `style={{ height: '${pct}%' }}`을 줘봐야 CSS 스펙상 퍼센트 높이는
  "정해진 높이를 가진 부모"를 기준으로만 계산되므로 기준이 없어 0으로 접혀 막대가 아예
  안 그려짐. e2e 테스트(`analysis.spec.ts`)는 텍스트 존재 여부만 확인해서 이 렌더링
  버그를 못 잡았음 — 시각적 버그는 e2e 텍스트 검증만으론 한계가 있다는 걸 재확인.
- **수정**: 부모는 기본 stretch로 두고(`items-end` 제거), 각 열 안에서 라벨 두 줄을 뺀
  나머지를 `flex-1` "트랙" div로 따로 감싸 그 트랙(정해진 높이가 flex 계산으로 확정됨)
  기준으로만 막대의 `height:%`가 계산되게 구조 변경. `HBar`(가로 막대, width:% 기반)와
  홀 상세 탭의 티샷결과 스택 바는 원래부터 문제없었음(가로 폭은 기본적으로 "정해진 값"이라
  퍼센트 너비는 이런 이슈가 없음 — 세로 높이만 이 함정이 있었던 것).

### 다음 세션 시작 시

- 이 버그로 특별히 대기 중인 후속 과제는 없음. 다만 앞으로 세로 막대류(height:% 사용) UI를
  또 추가할 일이 있으면 이번 트랙 분리 패턴을 그대로 재사용할 것.


## 130. 화면 전환 로딩 인디케이터 추가 (2026-07-30)

재홍님이 "화면 간 이동 시에 모래시계 같은 로딩 표시가 추가됐으면 좋겠다"고 요청 — 먼저
기존 로딩 관련 코드를 조사(서브에이전트 활용)해 의견 제시: 이 프로젝트는 아직 Next.js
App Router의 네비게이션 전용 로딩 기능(`loading.tsx`)을 전혀 안 쓰고 있고, 대신 버튼
클릭형 로딩만 두 패턴(전체화면 오버레이 / 버튼 텍스트 전환)으로 존재. 화면 이동 자체엔
`loading.tsx`가 Next.js 표준 방식이라고 추천 → AskUserQuestion으로 "중앙 원형 스피너
(loading.tsx)" vs "상단 진행바(별도 구현 필요)" 확인 → **중앙 원형 스피너** 선택.

- **`app/src/app/loading.tsx` 신규**: 루트에 하나만 두면 모든 화면 이동에서 목적지
  페이지의 서버 데이터 조회(대부분 `auth()`+Prisma) 동안 자동으로 뜬다 — Next.js가
  Suspense 경계로 처리해줘서 클릭 이벤트/로딩 상태를 따로 관리할 필요 없음. 스피너는
  기존 `PublicDataSyncCard`/`GeocodeBatchCard`의 `animate-spin` 링 방식을 그대로
  재사용하되, 그쪽은 어두운 오버레이 위 흰 링이고 이쪽은 배경 없는 페이지 전체라
  앱 팔레트(`border-card-bg border-t-primary`)로 톤을 맞춤.
- `doc/pages.md` 공통 컴포넌트 절에 반영.
- 검증: `npx tsc --noEmit` 클린. `loading.tsx`는 네비게이션 타이밍에 좌우되는 시각적
  요소라 e2e로 안정적으로 검증하기 어려워 스킵 — **재홍님 로컬에서 화면 전환 시 실제로
  스피너가 보이는지 눈으로 확인 필요**(특히 관리자 화면처럼 조회가 좀 걸리는 곳에서
  체감이 클 것).

### 다음 세션 시작 시

- 로컬 확인 결과("스피너가 잘 보이는지", 너무 짧게 스쳐 지나가진 않는지 등) 공유받으면
  필요시 최소 노출시간 등 튜닝 논의. 특별한 문제 없으면 완료로 종료.


## 131. 추이 탭 타수 라벨 위치 버그 수정 (2026-07-30)

재홍님이 "스피너 표시 됨. 분석화면 라운드 추이 부분에 타수가 막대 바로 위쪽에 표시돼야
하는데 지금은 막대 길이와 상관없이 모두 동일 높이에 표시되고 있음" 보고.

- **원인**: 타수 라벨(`<span>{p.strokes}</span>`)이 막대를 담은 트랙(flex-1) "바깥",
  열(column)의 맨 위 고정 위치에 있어서 — 129번에서 막대 자체는 고쳤지만 라벨은 여전히
  열 상단에 고정된 채였음. 그래서 막대가 짧든 길든 라벨은 항상 같은 y좌표에 떠 있었음.
- **수정**: 라벨을 트랙 "안"으로 옮겨 막대와 같은 `flex-col justify-end` 그룹으로 묶음 —
  이러면 라벨+막대가 한 덩어리로 트랙 바닥에 붙고, 막대가 길수록 그 덩어리 전체가 위로
  밀려 올라가면서 라벨도 막대 길이에 비례해 같이 올라간다. 라벨이 트랙 위쪽 경계를 넘지
  않도록 막대 최대 높이도 90%→80%로 살짝 낮춤(20~80% 정규화로 변경).
- `npx tsc --noEmit` 클린. 시각적 버그라 e2e로는 못 잡음 — 재홍님 육안 확인 필요.

### 다음 세션 시작 시

- 확인 결과 이상 없으면 이 항목도 완전 종료. 특별히 대기 중인 후속 과제 없음.


## 132. 스코어 등록 Step1→Step2 로컬 로딩 오버레이 추가 (2026-07-30)

재홍님이 스피너 안 뜨는 화면 3곳 보고 → 조사(서브에이전트) 결과 원인이 서로 다름을 확인:
1) **스코어 등록 Step1→Step2**: `/rounds/new` 경로 그대로, `?step=` 쿼리만 바뀌는 구조라
   전역 `app/loading.tsx`(라우트 세그먼트 전환에만 반응)가 구조적으로 안 뜨는 케이스 —
   확실한 버그.
2) **골프장 상세, 루프·Par 입력**: 코드는 대시보드→기록분석(정상 동작 확인됨)과 동일한
   `<Link>` 패턴이라 차이를 못 찾음. `findUnique` 단건 조회라 응답이 빨라 스피너가 뜰
   새도 없이 지나가는 것으로 추정.

AskUserQuestion으로 확인 → **1)은 이 화면에만 로컬 오버레이 추가, 2)는 그대로 둔다**
(둘 다 추천안 그대로 선택).

- **`RoundStep1.tsx` 수정**: `useTransition` 도입, `router.push`를 `startTransition()`으로
  감싸 `isPending`이 "다음 화면(Step2) 렌더 준비 끝날 때까지" true로 유지되게 함(그냥
  `router.push`만 호출하면 함수 자체는 동기적으로 끝나 대기 상태를 못 잡음). `isPending`일
  때 기존 관리자 화면(`PublicDataSyncCard`/`GeocodeBatchCard`)과 동일한 전체화면 오버레이
  스타일(`fixed inset-0 z-50 bg-black/50` + 흰 링 스피너)을 그대로 재사용. 버튼도
  "이동 중..."으로 텍스트 전환 + disabled. 기존 `checking`(중복확인 fetch 대기)과는 별개
  상태로 분리 — 중복확인 중엔 "확인 중...", 그 다음 실제 이동 중엔 "이동 중..."으로 순차
  전환됨. `goNext()` 내부 로직도 try/catch에서 중복 코드(파라미터 조립을 두 번 하던 것)를
  하나로 정리.
- `npx tsc --noEmit` 클린. e2e(`rounds-new.spec.ts`)는 버튼 클릭 자체는 그대로라 영향 없음
  (별도 재실행 안 함, 재홍님 로컬에서 필요시 확인).

### 다음 세션 시작 시

- 재홍님 로컬 확인(오버레이가 실제로 보이는지) 결과 공유받으면 종료. 다른 화면에서도
  같은 "경로는 그대로, 쿼리/상태만 바뀌는" 네비게이션 패턴이 발견되면 이번
  `useTransition` + 전체화면 오버레이 패턴을 그대로 재사용할 것.


## 133. 문서 동기화 점검 — pages.md/개발리스트.md 누락분 보완 (2026-07-30)

재홍님이 "작업 내용 memory.md 파일과 관련 파일들에 업데이트 해줘"로 요청. memory.md는
125~132번 항목마다 매번 즉시 갱신·커밋해와서 이미 최신 상태였음(git status 확인
결과 대기 중인 변경 없음) — 점검해보니 132번(Step1→Step2 로딩 오버레이)이 memory.md엔
기록됐지만 `doc/pages.md`(7-1 상태 항목)·`doc/개발리스트.md`(7-1 표)에는 아직 반영이
안 돼 있어 이번에 보완:

- `doc/pages.md` 7-1절 "상태" 항목에 이동 중 로딩 오버레이 내용 한 문장 추가.
- `doc/개발리스트.md` 7-1 표에 "Step2 이동 중 로딩 오버레이" 행 신규 추가.
- 129/131번(추이 탭 CSS 버그 2건)은 순수 렌더링 버그 수정이라 화면 스펙 자체는 안
  바뀜 — `pages.md`/`개발리스트.md`에 별도 항목을 추가하지 않고 memory.md 로그로만
  남기는 게 맞다고 판단(다른 유사 버그 수정 사례들과 동일한 처리 기준).
