...
- **화면 단위 밖에서 남은 작업(우선순위 순서 없음, 사용자 지정 대기)**:
  ...
  2. 좌표 결측 골프장 지오코딩 배치 — 원본 좌표값 자체가 없는 소수 골프장은 여전히 "위치 확인 중" 상태.
  3. 카카오 OAuth 실제 로그인 테스트 — 사용자가 Kakao Developers에서 앱 등록/키 발급 후 로컬 테스트 예정(구글은 완료).
  4. 하이원CC 홀별 Par 자료 미확보(부차 데이터 이슈) — 사용자가 공식 스코어카드 확보 시 재요청 예정.
  5. **기상청 날씨 API(77번, 2026-07-22)는 코드 구현까지는 완료했으나 `.env`의 `WEATHER_API_KEY`가 아직 빈 값** — 사용자가 data.go.kr에서 "기상청_단기예보 조회서비스" 활용신청 후 키를 채워 넣어야 실제 동작. 그 전까지는 7-1 미리보기/`weatherSnapshot` 저장 모두 "날씨 정보 없음"으로만 보임(정상, 에러 아님).
...
- 세부 이력은 아래 8~77번 항목(시간순 작업 로그)에 모두 기록되어 있음.
